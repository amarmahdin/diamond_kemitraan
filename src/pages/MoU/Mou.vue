...
import { MOU_STEPS } from '@/constants/navigation'
...
function onDocumentClick(event) {
  if (!dropContainer.value?.contains(event.target)) {
    isDropdownOpen.value = false
  }
}
...
<template>
  <WizardPageLayout
    :breadcrumb="[{ label: 'Buat' }, { label: 'MoU/NDA' }]"
    title="MoU/NDA"
  ...
  >
    ...
    <div class="grid gap-6 px-4 md:grid-cols-2">
      <div ref="dropContainer" class="relative">
        <label class="form-label">
          MoU / NDA <span class="text-status-red">*</span>
        </label>
        <button
          type="button"
          class="mt-2 flex h-10 w-full items-center justify-between border border-surface-line px-4 text-sm"
          :class="isDropdownOpen ? 'rounded-t-lg' : 'rounded-lg'"
          @click.stop="toggleDropdown"
        >
          <span :class="selectedOption ? 'text-ink' : 'text-ink-faint'">
            {{ selectedOption || 'Pilih MoU/NDA' }}
          </span>
        ...
        >
          <li
            class="cursor-pointer border-b border-surface-line px-4 py-2"
            :class="selectedOption === 'MoU' ? 'bg-brand text-white' : 'hover:bg-brand-soft hover:text-brand'"
            @click="selectOption('MoU')"
          >
            MoU
          </li>
          <li
            class="cursor-pointer px-4 py-2"
            :class="selectedOption === 'NDA' ? 'bg-brand text-white' : 'hover:bg-brand-soft hover:text-brand'"
            @click="selectOption('NDA')"
          >
            NDA
          </li>
        </ul>
      ...
      <div>
        <label class="form-label" for="mou-date">
    ...
    <FormNavButtons next-to="/Ruanglingkup" />
  </WizardPageLayout>
</template>
